...
const execute = async (bot, interaction) => {
  try {
    ...
    if (guild.vanityURLCode) {
      embed.fields.push(
        { name: "Vanity URL", value: "https://discord.gg/" + guild.vanityURLCode, inline: true },
        { name: "Vanity Uses", value: guild.vanityURLUses != null ? guild.vanityURLUses.toString() : "None", inline: true }
      );
    }

    embed.fields.push(
      { "name": "Server Created", "value": new Date(guild.createdTimestamp).toString() || "N/A" },
      { "name": `${bot.name} Joined`, "value": new Date(guild.joinedTimestamp).toString() || "N/A" }
    );
