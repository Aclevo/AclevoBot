...
const execute = async (bot, interaction) => {
  ...
  const invite = oInvite.replace(/[\/]/g, '').replace("https:", "").replace("http:", "").replace("discord.gg", "");
  ...
  try {
    ...
    if (res.status !== 200 && res.status !== 404) {
      ...
      if (inviteData.message) {
        embed = {
          description: `${bot.config.system.emotes.error} **${inviteData.message}**`,
          color: bot.config.colors.red,
        };
      } else {
        embed = {
          title: `${bot.config.system.emotes.information} **${inviteData.guild.name} (${inviteData.guild.id})**`,
          color: bot.config.colors.blue,
          fields: [
            { name: "Channel", value: `#${inviteData.channel.name} (${inviteData.channel.id}) | <#${inviteData.channel.id}>` },
            { name: "Members", value: "About " + inviteData.approximate_member_count },
            { name: "Features", value: (inviteData.guild.features.length > 0) ? "`" + inviteData.guild.features.join("`, `") + "`" : "No special features :(" },
            { name: "Boosts", value: inviteData.guild.premium_subscription_count.toString(), inline: true },
          ],
        };

        if (inviteData.guild.description != null) embed.description = inviteData.guild.description;
        if (inviteData.guild.vanity_url_code) embed.fields.push({ name: "Vanity URL", value: "https://discord.gg/" + inviteData.guild.vanity_url_code, inline: true });
        if (inviteData.expires_at) embed.fields.push({ name: "Expires in", value: bot.functions.TStoHR((new Date(inviteData.expires_at).getTime()) - new Date().getTime()), inline: true }); // expires_at Time minus the current Time

        if (inviteData.inviter) {
          const user = await bot.client.users.fetch(inviteData.inviter.id);
          embed.author = {
            name: `Invite Created by ${user.username} (${user.id})`,
            icon_url: user.displayAvatarURL({ extension: "png", dynamic: true, size: 1024 })
          };
        }
    ...
    bot.logger.error("SYS", `Failed fetch of invite: ${oInvite} | ${error.message}`);
    ...
    await interaction.reply({
      embeds: [{
        description: `${bot.config.system.emotes.error} **${messageData}**`,
        color: bot.config.colors.red,
      }],
    });
  }
