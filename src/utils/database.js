...
class Database {
  ...
  async init(dbCfg) {
    ...
    this.bot.logger.debug(
      "DATABASE",
      `Connecting to database ${dbCfg.database}...`,
    );

    this.bot.db.connection = new Sequelize(
      dbCfg.database,
      dbCfg.username,
      dbCfg.password,
      dbCfg.dbCfg,
    ...
    await this.bot.db.connection
      ...
      .then(async () => {
        ...
        this.bot.logger.debug(
          "DATABASE",
          `Syncing the database ${dbCfg.database}...`,
        );
        await this.bot.db.connection
          .sync()
          .then(() => {
          ...
          .catch((err) => {
            this.bot.logger.error(
              "DATABASE",
              `DB Sync ${dbCfg.database}: NOT OK - ${err.message}`,
            );
      ...
      .catch((err) => {
        this.bot.logger.error(
          "DATABASE",
          `DB Connect ${dbCfg.database}: NOT OK - ${err.message}`,
        );
        console.log(err.stack);
      });
