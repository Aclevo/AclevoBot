...
class Functions {
  ...
  clearCache = (mod) => {
    // Bun doesn't use require.cache, so we'll skip this for now
    // In Bun, modules are cached differently
  };
  ...
  genError = async (interaction, Ex) => {
    ...
    this.bot.logger.error(
      "SYSTEM",
      `Unexpected error during execution of ${interaction.commandName} for ${interaction.user.id}`,
    );
    ...
    if (Ex.stack && this.bot.config.debug)
      errEmbed.fields.push({
        name: "Stacktrace",
        value:
          Ex.stack.length >= MAX_STACK_DETAILS
            ? Ex.stack.substring(0, MAX_STACK_DETAILS) + moreDetails
            : Ex.stack,
      });
    ...
    errEmbed.fields.push({
      name: "Support",
      value: this.bot.config.discord.supportInviteBase,
      inline: refID != null,
    });
