...
export default defineEvent({
  ...
  run: async (bot, params) => {
    ...
    bot.logger.info(
      "DISCORD",
      `${user.tag} subscribed to event ${scheduledEvent.name} in ${guild.name} (${guild.id})`,
    );

    const embed = new EmbedBuilder()
      .setColor(bot.config.colors.green)
      .setTitle("Event Subscription Added")
      .setDescription(`A user subscribed to a scheduled event in ${guild.name}`)
      .addFields(
    ...
    try {
      ...
      bot.logger.warn(
        "DISCORD",
        `Could not send scheduled event user add message: ${error.message}`,
      );
    }
  },
});
